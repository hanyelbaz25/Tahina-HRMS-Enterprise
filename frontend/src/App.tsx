...
function DocumentsPage({ token, isAdmin }: { token: string; isAdmin: boolean }) {
  ...
  const selectedEmployee = employees.find((x) => x.id === form.employee_id);
  const expiringCount = visible.filter((x) => x.days_remaining >= 0 && x.days_remaining <= 90).length;
  const expiredCount = visible.filter((x) => x.days_remaining < 0).length;
  ...
  return (
    <>
      ...
      <div className="enterprise-stats-grid">
        <div className="enterprise-stat-card"><span>إجمالي المستندات</span><strong>{visible.length}</strong><small>عقود وإقامات مسجلة</small></div>
        <div className="enterprise-stat-card warning"><span>قريبة من الانتهاء</span><strong>{expiringCount}</strong><small>خلال 90 يومًا</small></div>
        <div className="enterprise-stat-card danger"><span>منتهية</span><strong>{expiredCount}</strong><small>تحتاج إلى إجراء</small></div>
      </div>
      <form className="enterprise-form" onSubmit={save}>
        <div className="enterprise-section-title"><div><strong>بيانات الموظف</strong><span>حدد الموظف المرتبط بالمستند</span></div></div>
        <label className="enterprise-field enterprise-field-wide"><span>البحث عن موظف</span><input type="search" placeholder="اكتب اسم الموظف أو الكود" value={search} onChange={(e) => { const value=e.target.value; setSearch(value); const q=value.trim().toLowerCase(); const first=employees.find((x)=>!q || x.full_name_ar.toLowerCase().includes(q) || x.employee_no.toLowerCase().includes(q)); setForm({...form,employee_id:first?.id ?? 0}); }} /></label>
        <label className="enterprise-field enterprise-field-wide"><span>الموظف</span><select value={form.employee_id} onChange={(e)=>setForm({...form,employee_id:Number(e.target.value)})}>{filteredEmployees.map((x)=><option key={x.id} value={x.id}>{x.employee_no} - {x.full_name_ar}</option>)}</select></label>
        {selectedEmployee && <div className="employee-summary enterprise-field-full"><div><b>{selectedEmployee.full_name_ar}</b><span>{selectedEmployee.employee_no}</span></div><div><small>الفرع</small><strong>{selectedEmployee.branch_name}</strong></div><div><small>المسمى الوظيفي</small><strong>{selectedEmployee.job_title || "—"}</strong></div></div>}

        <div className="enterprise-section-title"><div><strong>بيانات العقد أو الإقامة</strong><span>أدخل التفاصيل والتواريخ بدقة</span></div></div>
        <label className="enterprise-field"><span>نوع المستند</span><select value={form.document_type} onChange={(e)=>setForm({...form,document_type:e.target.value})}><option value="contract">عقد عمل</option><option value="iqama">إقامة</option></select></label>
        <label className="enterprise-field"><span>رقم المستند</span><input placeholder="رقم العقد أو الإقامة" value={form.document_no} onChange={(e)=>setForm({...form,document_no:e.target.value})}/></label>
        <label className="enterprise-field"><span>تاريخ البداية</span><input type="date" value={form.start_date} onChange={(e)=>setForm({...form,start_date:e.target.value})}/></label>
        <label className="enterprise-field"><span>تاريخ الانتهاء</span><input type="date" value={form.expiry_date} onChange={(e)=>setForm({...form,expiry_date:e.target.value})}/></label>
        <label className="enterprise-field enterprise-field-full"><span>الملاحظات</span><input placeholder="أي ملاحظات إضافية" value={form.notes} onChange={(e)=>setForm({...form,notes:e.target.value})}/></label>

        <div className="enterprise-section-title"><div><strong>المرفقات</strong><span>PDF أو صورة بحد أقصى مناسب</span></div></div>
        <label className="file-drop enterprise-field-full"><Upload size={25}/><strong>{file ? file.name : "اسحب الملف هنا أو اضغط للاختيار"}</strong><span>PDF, PNG, JPG, JPEG</span><input type="file" accept=".pdf,.png,.jpg,.jpeg" onChange={(e)=>setFile(e.target.files?.[0] || null)}/></label>
        <div className="enterprise-actions enterprise-field-full"><button disabled={!form.employee_id}>حفظ المستند</button></div>
      </form>
      <div className="search">
...
function LeavesPage({ token, isAdmin }: { token: string; isAdmin: boolean }) {
  ...
  const selectedEmployee = employees.find((x) => x.id === form.employee_id);
  const selectedBalance = balances.find((x) => x.employee_id === form.employee_id);
  const requestedDays = Math.max(1, Math.floor((new Date(form.end_date).getTime() - new Date(form.start_date).getTime()) / 86400000) + 1);
  const pendingCount = items.filter((x) => x.status === "pending").length;
  const activeCount = items.filter((x) => x.status === "approved" && x.start_date <= today && x.return_date > today).length;
  async function save(e: FormEvent) {
    e.preventDefault();
    try {
  ...
  return (
    <>
      ...
      {message && <div className="success">{message}</div>}
      <div className="enterprise-stats-grid">
        <div className="enterprise-stat-card"><span>طلبات السنة</span><strong>{items.length}</strong><small>كل طلبات {year}</small></div>
        <div className="enterprise-stat-card warning"><span>قيد المراجعة</span><strong>{pendingCount}</strong><small>بانتظار الاعتماد</small></div>
        <div className="enterprise-stat-card success"><span>في إجازة الآن</span><strong>{activeCount}</strong><small>موظفون حاليًا</small></div>
      </div>
      <form className="enterprise-form" onSubmit={save}>
        <div className="enterprise-section-title"><div><strong>بيانات الموظف</strong><span>اختر الموظف لمراجعة الرصيد قبل التسجيل</span></div></div>
        <label className="enterprise-field enterprise-field-wide"><span>البحث عن موظف</span><input type="search" placeholder="اكتب اسم الموظف أو الكود" value={search} onChange={(e)=>{const value=e.target.value;setSearch(value);const normalized=value.trim().toLowerCase();const firstMatch=employees.find((employee)=>!normalized || employee.full_name_ar.toLowerCase().includes(normalized) || employee.employee_no.toLowerCase().includes(normalized));setForm({...form,employee_id:firstMatch?.id ?? 0});}}/></label>
        <label className="enterprise-field enterprise-field-wide"><span>الموظف</span><select value={form.employee_id} onChange={(e)=>setForm({...form,employee_id:Number(e.target.value)})}>{filtered.map((x)=><option key={x.id} value={x.id}>{x.employee_no} - {x.full_name_ar}</option>)}</select></label>
        {selectedEmployee && <div className="employee-summary enterprise-field-full"><div><b>{selectedEmployee.full_name_ar}</b><span>{selectedEmployee.employee_no}</span></div><div><small>الفرع</small><strong>{selectedEmployee.branch_name}</strong></div><div><small>الرصيد المتبقي</small><strong>{selectedBalance?.remaining ?? "—"} يوم</strong></div></div>}

        <div className="enterprise-section-title"><div><strong>طلب الإجازة</strong><span>حدد النوع والفترة والسبب</span></div></div>
        <label className="enterprise-field"><span>نوع الإجازة</span><select value={form.leave_type} onChange={(e)=>setForm({...form,leave_type:e.target.value})}><option value="annual">سنوية</option><option value="sick">مرضية</option><option value="unpaid">بدون راتب</option><option value="unexcused">غياب غير مبرر</option></select></label>
        <label className="enterprise-field"><span>تاريخ البداية</span><input type="date" value={form.start_date} onChange={(e)=>setForm({...form,start_date:e.target.value})}/></label>
        <label className="enterprise-field"><span>تاريخ النهاية</span><input type="date" value={form.end_date} onChange={(e)=>setForm({...form,end_date:e.target.value})}/></label>
        <div className="leave-days-card"><span>مدة الإجازة</span><strong>{requestedDays}</strong><small>يوم</small></div>
        <label className="enterprise-field enterprise-field-full"><span>السبب أو الملاحظات</span><input placeholder="اكتب سبب الإجازة أو أي ملاحظات" value={form.reason} onChange={(e)=>setForm({...form,reason:e.target.value})}/></label>
        <div className="enterprise-actions enterprise-field-full"><button disabled={!form.employee_id}>تسجيل طلب الإجازة</button></div>
      </form>
      <div className="search">
        <label>
          السنة:{" "}
      ...
      <div className="table-section-heading"><div><strong>أرصدة الإجازات السنوية</strong><span>ملخص الاستحقاق والمستخدم والمتبقي لعام {year}</span></div></div>
      <div className="table-wrap">
        <table>
          <thead>
            <tr>
...
function EmployeeChargesPage({ token, isAdmin }: { token: string; isAdmin: boolean }) {
  ...
  return (
    <>
      ...
      {error && <div className="error">{error}</div>}
      {message && <div className="success">{message}</div>}
      <form className="crud-form employee-form financial-movement-form" onSubmit={save}>
        <div className="form-section-title">
          <strong>بيانات الموظف</strong>
          <span>اختر الموظف والفرع المرتبط بالحركة المالية</span>
        </div>

        <label className="form-field">
          <span className="form-field-label">البحث عن الموظف</span>
          <input
            type="search"
            placeholder="اكتب أول حروف الاسم أو جزءًا من الكود"
        ...
        <label className="form-field">
          <span className="form-field-label">الموظف المختار</span>
          <select
            value={form.employee_id}
            onChange={(e) =>
              setForm({ ...form, employee_id: Number(e.target.value) })
        ...
        <label className="form-field">
          <span className="form-field-label">الفرع</span>
          <select
            value={form.branch_id}
            onChange={(e) =>
              setForm({ ...form, branch_id: Number(e.target.value) })
        ...
        <div className="form-section-title">
          <strong>تفاصيل الحركة المالية</strong>
          <span>حدد نوع الحركة ومصدرها وتاريخها وقيمتها</span>
        </div>

        <label className="form-field">
          <span className="form-field-label">تاريخ الحركة</span>
          <input
            type="date"
        ...
        <label className="form-field">
          <span className="form-field-label">نوع الحركة</span>
          <select
        ...
        <label className="form-field">
          <span className="form-field-label">مصدر الصرف</span>
          <select
        ...
        <label className="form-field">
          <span className="form-field-label">القيمة</span>
          <input
            type="number"
            min="0.01"
            step="0.01"
        ...
        <div className="form-section-title">
          <strong>التطبيق والتحصيل</strong>
          <span>حدد بداية الخصم أو الإضافة وعدد الأقساط والملاحظات</span>
        </div>

        <label className="form-field">
          <span className="form-field-label">شهر بدء التطبيق</span>
          <input
            type="month"
        ...
        <label className="form-field">
          <span className="form-field-label">عدد الأقساط</span>
          <input
            type="number"
            min="1"
            max="60"
        ...
        <label className="form-field form-field-wide">
          <span className="form-field-label">ملاحظات</span>
          <input
...
function PayrollPage({ token, isAdmin }: { token: string; isAdmin: boolean }) {
  ...
  return (
    <>
      ...
      <form className="crud-form employee-form payroll-form" onSubmit={save}>
        <div className="form-section-title">
          <strong>بيانات الموظف والمسير</strong>
          <span>اختر الموظف والفرع والشهر قبل إدخال تفاصيل الراتب</span>
        </div>
        <label className="form-field form-field-wide">
          <span className="form-field-label">الموظف</span>
          <select value={form.employee_id} onChange={(e) => setForm({ ...form, employee_id: Number(e.target.value) })}>
            {employees.map((x) => (<option key={x.id} value={x.id}>{x.employee_no} - {x.full_name_ar}</option>))}
          </select>
        </label>
        <label className="form-field">
          <span className="form-field-label">الفرع</span>
          <select value={form.branch_id} onChange={(e) => setForm({ ...form, branch_id: Number(e.target.value) })} required>
            {branches.map((branch) => (<option key={branch.id} value={branch.id}>{branch.name_ar}</option>))}
          </select>
        </label>
        <label className="form-field">
          <span className="form-field-label">شهر المسير</span>
          <input type="month" value={form.payroll_month} onChange={(e) => setForm({ ...form, payroll_month: e.target.value })} />
        </label>
        <div className="form-section-title">
          <strong>أيام العمل والإضافي</strong>
          <span>أدخل أيام العمل وأيام أو قيمة العمل الإضافي</span>
        </div>
        <label className="form-field">
          <span className="form-field-label">أيام العمل</span>
          <input type="number" min="0" step="0.5" value={form.work_days} onChange={(e) => setForm({ ...form, work_days: Number(e.target.value) })} />
        </label>
        <label className="form-field">
          <span className="form-field-label">أيام الإضافي</span>
          <input type="number" min="0" step="0.5" value={form.addition_days || ""} onChange={(e) => setForm({ ...form, addition_days: Number(e.target.value) })} />
        </label>
        <label className="form-field form-field-wide">
          <span className="form-field-label">مبلغ الإضافي</span>
          <input type="number" min="0" step="0.01" value={form.addition_amount || ""} onChange={(e) => setForm({ ...form, addition_amount: Number(e.target.value) })} />
        </label>
        <div className="form-section-title">
          <strong>الغياب والخصومات</strong>
          <span>سجّل أيام الغياب وأي خصومات مالية إضافية</span>
        </div>
        <label className="form-field">
          <span className="form-field-label">أيام الغياب</span>
          <input type="number" min="0" step="0.5" value={form.deduction_days || ""} onChange={(e) => setForm({ ...form, deduction_days: Number(e.target.value) })} />
        </label>
        <label className="form-field">
          <span className="form-field-label">الخصومات المالية</span>
        ...
        <div className="form-actions-row">
          <button disabled={periodStatus.status === "approved"}>حفظ المسير</button>
        </div>
      </form>
      <div className="search">
        <label>
